...
import {
  ...
  changeTheme,
  changeSidebarThemeToggle,
} from "Slices/theme/reducer";
...
import avatar01 from "assets/images/avatar/male.jpeg";
...
const TopBar = () => {
  ...
  const [isUpdateSubscription, setIsUpdateSubscription] =
  ...
  const toggleModeTheme = () => {
    const newTheme =
      themeType === THEME_MODE.LIGHT ? THEME_MODE.DARK : THEME_MODE.LIGHT;
  ...
  const toggleSidebar = () => {
    ...
    dispatch(
      changeSidebarThemeToggle(
        newToggleValue ? THEME_SIDEBAR_TOGGLE.TRUE : THEME_SIDEBAR_TOGGLE.FALSE
      )
    );
  };
  return (
